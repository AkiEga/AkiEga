## Hi there!  👋 I'm AkiEga

I'm a developer based in Japan 🇯🇵

### 🔭 What I'm up to
- Working on embedded systems development
- Building projects and improving my coding skills
- Exploring new technologies in my free time

### 🌱 Interests
- Embedded systems
- Software development
- Open source contributions
- Continuous learning

### Tools I've Built

#### VS Code Extensions
- [file-open-history](https://github.com/AkiEga/file-open-history) — Tracks and displays the history of files you open in VS Code
- [vscode-extension-ascii-sketch](https://github.com/AkiEga/vscode-extension-ascii-sketch) — VS Code extension for creating and editing ASCII art with canvas, box drawing, and alignment tools
- [vscode-extension-section-info](https://github.com/AkiEga/vscode-extension-section-info) — Copies selected code section details and formatted git context to the clipboard in customizable templates
- [vscode-extension-svg-sketch](https://github.com/AkiEga/vscode-extension-svg-sketch) — A VS Code extension for creating and editing SVG diagrams with a canvas-based editor
- [vscode-extension-tree](https://github.com/AkiEga/vscode-extension-tree) — Shows a folder's tree command output in VS Code's editor or webview from the Explorer sidebar
- [vscode-msvs-proj-manager](https://github.com/AkiEga/vscode-msvs-proj-manager) — Parses Visual Studio solution files in VS Code and opens terminals in each project's root folder
#### Embedded Systems / Hardware
- [pico_arcade_controller](https://github.com/AkiEga/pico_arcade_controller) — Arcade joystick controller built with Raspberry Pi Pico (USB HID). Supports mouse cursor movement, long-press acceleration, and window switching via rapid left/right inputs.
- [ega-right-kb](https://github.com/AkiEga/ega-right-kb) — Custom right-side keyboard powered by Raspberry Pi Pico with GPIO key mapping and HID support.

###  How to reach me
Feel free to connect with me here on GitHub! 

### ⚡ Fun fact
I'm passionate about technology and enjoy solving problems through code.

---

**Note:** All repositories here are my personal projects and are not affiliated with any organization I belong to. 

💡 *Always learning, always coding!*
